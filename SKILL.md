...
python scripts/search.py list funcs <对象名>   # 列出指定对象的所有方法 (含继承链, 含描述)
python scripts/search.py list events         # 列出所有事件函数名(含描述)
```
...
python scripts/search.py list packages             # 列出所有源码包
python scripts/search.py list examples             # 列出所有示例
python scripts/search.py list events button        # 列出指定对象所有事件(含描述)
python scripts/search.py list funcs button         # 列出指定对象所有方法(含描述)
python scripts/search.py list events               # 列出所有事件函数名(含描述)
```
...
| 查看对象的所有方法 | `python scripts/search.py list funcs <对象名>` | 含继承链上的所有方法名(含事件方法) |
...
- 不在 UI 线程操作 UI 导致崩溃, 对界面库元素的修改操作必须在 UI 线程执行，元素并不是线程安全的, 可使用 `xc.UI` 函数在UI线程执行操作, `xc.Auto` 函数是会自动判断是否在 UI 线程
...
- 有些元素需要创建数据适配器后才能使用, 不创建就会报错: List, ListView, ListBox, Tree, CombBox. 怎么创建可查看它们的例子
